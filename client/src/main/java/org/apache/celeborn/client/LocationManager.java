...
public class LocationManager {
  ...
  private class PartitionLocationList {
    ...
    // the size of locations already used（included unusable locations
    int used = 0;
    // the size of locations to be used
    int size = 0;
    int maxEpoch = -1;
    ...
    public PartitionLocation getLatestPartitionLocation() {
      try {
        readLock.lock();
  ...
  // shuffleId -> partitionId -> PartitionLocationList
  ...
  public int getMaxEpoch(int shuffleId, int partitionId) {
    PartitionLocationList locationList = reducePartitionMap.get(shuffleId).get(partitionId);
    return locationList.getMaxEpoch();
  }

  public PartitionLocation getLatestPartitionLocation(int shuffleId, int partitionId) {
    PartitionLocationList locationList = reducePartitionMap.get(shuffleId).get(partitionId);
    return locationList.getLatestPartitionLocation();
  }
}
