...
class LifecycleManager(val appUniqueId: String, val conf: CelebornConf) extends RpcEndpoint
  ...
  private def handleGetShuffleIdForApp(
    ...
    // shuffleIds： LinkedHashMap of (app shuffle identifier, (shuffle id, fetch status))
    if (shuffleIds == null) {
      ...
      if (isWriter) {
        shuffleIds.get(appShuffleIdentifier) match {
          ...
          case None =>
            ...
            //  2. UNORDERED: The RDD output is always the same data set but the order can be different after a rerun.
